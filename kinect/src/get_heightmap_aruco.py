...
def diagonal_camera():
    ...
    # 0.99862574  0.00463203 -0.05220319
    # ロール、ピッチ、ヨーの角度を90度ずつ設定（ラジアン単位）
    # roll_angle = np.radians(0)
    roll_angle = np.deg2rad(10)#ヒューリスティックに探索
    ...
    # workspace_limits = np.asarray([[-1, 2], [-2, 1], [-1, 3]])
    workspace_limits = np.asarray([[-3, 1], [-3, 1], [-1, 3]])
    ...
    # if True:
        ...
        area = get_workspace_area(rgb_image)
        ...
        top_down_area, ret_depth = get_heightmap(area, depth_image, mtx, cam_pose, workspace_limits, heightmap_resolution)
        # plt.imshow(top_down_rgb);plt.show()
        # import ipdb;ipdb.set_trace()
        x_area, y_area = np.where(np.all(top_down_area==[255, 0, 0], axis=-1))
        xmin = min(x_area)
        xmax = max(x_area)
        ymin = min(y_area)
        ymax = max(y_area)
        xy_area = np.array([xmin, xmax, ymin, ymax])
        np.save('/root/catkin_ws/src/kinect/pose/diagonal_workspace_05_224.npy', xy_area)
        output = top_down_rgb[xmin:xmax, ymin:ymax]
        output = cv2.resize(output, (112,112))
        plt.imshow(output)
...
def top_down_camera():
    ...
    # if True:
        ...
        x_area, y_area = np.where(np.all(area==[255, 0, 0], axis=-1))
        xmin = min(x_area)
        xmax = max(x_area)
        ymin = min(y_area)
        ymax = max(y_area)
        xy_area = np.array([xmin, xmax, ymin, ymax])
        np.save('/root/catkin_ws/src/kinect/pose/top_down_workspace_05_224.npy', xy_area)
        depth_heightmap = depth_image[xmin:xmax, ymin:ymax]
        np.save("/root/catkin_ws/src/kinect/pose/top_down_base_height.npy", cv2.resize(depth_heightmap, (112, 112)))
        output = rgb_image[xmin:xmax, ymin:ymax]
        output = cv2.resize(output, (112,112))
        plt.imshow(output)
        # plt.imshow(top_down_rgb)
        plt.pause(1/30)
